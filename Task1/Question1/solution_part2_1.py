#To handle the second part of the problem, we need to identify both numerical digits and 
#digits written as words (like "one", "two", etc.). We'll adjust the logic so it can find the first and last digit from a line of text, 
#whether the digit is numerical or spelled out. Then, we'll convert these found digits (if spelled out) into their numeric equivalents, 
#form a two-digit number, and compute the sum.

#Steps:
# #Identify numerical digits (0-9) or spelled-out digits (one-nine) from each line.
# Find the first and last digit, whether it's numerical or spelled out.
...
# Let’s modify the code to account for these changes:

# Define a dictionary to map spelled-out digits to their numeric values
word_to_digit = {
...
def find_first_last_digit(line):
    """
    Finds the first and last digit in the given line, either as numeric or spelled out.
    Returns the numeric values as strings.
    """
    words = line.split()  # Split the line into words to help identify spelled-out digits
    first_digit = None
    last_digit = None
    
    # Find the first numeric digit or spelled-out digit
    for word in line.split():
        # Check if it's a number (single digit)
        if word.isdigit():
            first_digit = word
            break
        # Check if it's a spelled-out digit
        for key in word_to_digit:
            if key in word:
                first_digit = word_to_digit[key]
                break
        if first_digit:
            break
    
    # Find the last numeric digit or spelled-out digit by reversing the line
...
def sum_calibration_values(input_str):
    """
    This function calculates the sum of all calibration values in the input string,
    accounting for both numeric and spelled-out digits.

    Args:
        input_str (str): A string containing the calibration document.
    ...
    for line in lines:
        ...
        # If the line contains at least one digit, form the calibration value and add it to the total
...
# Example input
calibration_document = """eight9fhstbssrplmdlncmmqqnklb39ninejz
three656
...
#Explanation:
#Dictionary word_to_digit: Converts spelled-out digits ("one", "two", etc.) to their numeric equivalents ("1", "2", etc.).
#find_first_last_digit function: Loops through each line, identifies both numerical and spelled-out digits, and returns the first and last one found.
#sum_calibration_values function: Processes each line, extracts the first and last digits, forms a two-digit number, and adds it to the total.
#This code should now correctly compute the sum of calibration values from the input, including both numerical and spelled-out digits.