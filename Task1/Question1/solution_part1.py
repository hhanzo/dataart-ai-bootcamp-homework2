#To find the sum of all calibration values, we need to parse each line of the calibration document, 
#extract the first and last digits, combine them to form a two-digit number, 
# and then add up all these two-digit numbers.

#Here’s the approach to achieve this:
#Extract the first digit from each line of text.
#Extract the last digit from each line of text.
#Combine them to form a two-digit number.
#Sum these two-digit numbers across all lines.
...
#Input Format
#You need to replace the calibration_document string with your actual puzzle input.
#To get the final answer, you can run the code and print the result of the sum_calibration_values function with your puzzle input.
#Note: You should not include the example puzzle input in your actual solution. 
#Instead, use your puzzle input as the argument to the sum_calibration_values function.